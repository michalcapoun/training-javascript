...
// Input positions will always follow this pattern : f1 (i.e : Char(representing one of a-h)Number(representing one of 1-8) on chess board)
...
const bishop = (startPos, endPos, num_Moves) => {
    ...
    let a = startPos.charCodeAt(0) - 97
        , b = endPos.charCodeAt(0) - 97
        , x = +startPos[1]
        , y = +endPos[1];

    return (a + x) % 2 === (b + y) % 2
        && (num_Moves !== 1 || Math.abs(a - b) === Math.abs(x - y));
}
